...
// 表单验证schema
const editFitnessItemSchema = z.object({
...
type EditFitnessItemFormData = z.infer<typeof editFitnessItemSchema>

interface EditFitnessItemDialogProps {
  open: boolean
...
export function EditFitnessItemDialog({
  open,
  onOpenChange,
  onSuccess,
  item,
}: EditFitnessItemDialogProps) {
  ...
  const form = useForm<EditFitnessItemFormData>({
    resolver: zodResolver(editFitnessItemSchema),
    defaultValues: {
      name: '',
  ...
  useEffect(() => {
    if (item && open) {
      ...
      if (item.videos && item.videos.length > 0) {
        setSelectedVideos(item.videos.map(v => v.video))
  ...
  const onSubmit = async (data: EditFitnessItemFormData) => {
    if (!item) return
    ...
    try {
      ...
      const response = await fetch(`/api/fitness-items/${item.id}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`${showVideoSelector ? 'max-w-6xl' : 'max-w-4xl'} max-h-[90vh] overflow-hidden flex flex-col`}>
        <DialogHeader>
          <DialogTitle>编辑运动条目</DialogTitle>
          <DialogDescription>
            修改运动条目的信息
          </DialogDescription>
        </DialogHeader>
