...
// 更新运动提醒的验证schema
const updateFitnessReminderSchema = z.object({
...
export async function GET(
  ...
  try {
    ...
    const reminder = await prisma.fitnessReminder.findFirst({
      where: {
        id: params.id,
    ...
    if (!reminder) {
      return NextResponse.json({ error: "运动提醒不存在" }, { status: 404 });
    }
    ...
    console.error("获取运动提醒详情错误:", error);
    return NextResponse.json(
      { error: "获取运动提醒详情失败" },
      { status: 500 }
    );
...
export async function PUT(
  ...
  try {
    ...
    // 验证提醒是否存在且属于当前用户
    const existingReminder = await prisma.fitnessReminder.findFirst({
      where: {
        id: params.id,
        userId: session.user.id,
      },
    ...
    const validatedData = updateFitnessReminderSchema.parse(body);
    ...
    // 更新提醒
    const reminder = await prisma.fitnessReminder.update({
      where: { id: params.id },
      data: updateData,
      include: {
        item: {
    ...
    console.error("更新运动提醒错误:", error);
    return NextResponse.json({ error: "更新运动提醒失败" }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    // 删除提醒
    await prisma.fitnessReminder.delete({
      where: { id: params.id },
    });
    ...
    console.error("删除运动提醒错误:", error);
    return NextResponse.json({ error: "删除运动提醒失败" }, { status: 500 });
  }
}
